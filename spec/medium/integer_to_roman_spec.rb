...
require 'medium/integer_to_roman'
...
# subtractive forms are used: 4 (IV), 9 (IX), 40 (XL), 90 (XC), 400 (CD) and 900 
# (CM).
...
describe IntegerToRoman do
  ...
  describe 'integer_to_roman' do
    it "returns 'III' for 3" do
        expect(ir.integer_to_roman(3)).to eq('III')
    end
    it "returns 'MMMDCCXLIX' for 3749" do
        expect(ir.integer_to_roman(3749)).to eq('MMMDCCXLIX')
    end
    it "returns 'LVIII' for 58" do
        expect(ir.integer_to_roman(58)).to eq('LVIII')
    end
    it "returns 'MCMXCIV' for 1994" do
        expect(ir.integer_to_roman(1994)).to eq('MCMXCIV')
    end
  end
end